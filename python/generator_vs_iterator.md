...
Iterator is more of a low level approach.

Easiest way to make a custom iterator is by using a generator.

To implement an iterator there has to be a custom class with `__iter__` and `__next__`.

Iterator is a concept to make an iterable object.

Example: a function generating a fibonacci sequence can be a standard function
or be implemented as an iterator.

Iterator as well as generators have a low memory footprint.
